...
public class MecanumDrivetrain extends RobotPart {
    ...
    public void drive(double[] drivePower, double rotatePower) {
        // Normalizes drivePower theta, because the math only works between -90 and 270 degrees.
        if (drivePower[1] > 1.5 * Math.PI) {
            drivePower[1] -= 2 * Math.PI;
        } else if (drivePower[1] <= -0.5 * Math.PI) {
            drivePower[1] += 2 * Math.PI;
        }
